...
public class AlternateHuntingStrategy extends HuntingStrategy {
	public AlternateHuntingStrategy(Agent agent) {
		super(agent);
	}
	...
	public int execute(long hh_protein_need, int hunt_radius) {
			...
			do {
				...
				if (agent.getHsearchradius() < 51) {
					...
					if (dw == 0) {
						...
						// DC: Bug: In some cases, the "deer_fix" was actually reducing the number of deer found, limiting an agent to seeing only 1, even if there are
						// a lot more
						if (deer_fix > .50 && known_deer < 1) {
							known_deer = 1;
						}

						// DC: also changing the number of trips to 1
						hunt_cal += agent.calcTravelCal(agent.getHsearchradius(), 0,
								1.0);
						C_hunt += agent.calcTravelCal(agent.getHsearchradius(), 0,
								1.0)
								/ Village.WORK_CAL_MAN;

						// JAC 3/06
						deer_time += agent.calcTravelCal(agent.getHsearchradius(),
								0, 1.0)
								/ Village.WORK_CAL_MAN;
						DsearchCal += agent.calcTravelCal(agent.getHsearchradius(),
								0, 1.0);
						...
						if (known_deer != 0) {
							do {
								...
								// difficulty
								double dpercent_kill = Village.uniformDblRand(
										0.3, 0.75);
						...
						// Additionally the cost of accuiring these animals is
						// higher. Hunting Lagomophs
						// will require Village.LAG_HUNTERS (3) members of the
						...
						if (hunters >= Village.HUNTERS) {
							...
							if (agent.getCurrentProteinStorage() < hh_protein_need
								...
								if (known_rabbits < Village.RLAG_HUNT / 2) {
									known_rabbits = 0;
								} /* DC: BUG: THIS MAKES NO SENSE.  We can't find more animals than are there!
                                 else if (known_rabbits < Village.RLAG_HUNT + 1) {
									known_rabbits = Village.RLAG_HUNT + 1;
								} */

								/* DC: cutting down the search time
								hunt_cal += agent.calcTravelCal(
										agent.getHsearchradius(), 0, 2.0);
								C_hunt += agent.calcTravelCal(
										agent.getHsearchradius(), 0, 2.0)
										/ Village.WORK_CAL_MAN;
								...
								rabbit_time += agent.calcTravelCal(
										agent.getHsearchradius(), 0, 2.0)
										/ Village.WORK_CAL_MAN;
								RsearchCal += agent.calcTravelCal(
										agent.getHsearchradius(), 0, 2.0);
		...
		if (deer_time > 0) {
			agent.setDeer_return_rate(0);
			agent.setDeer_return_rate(agent.getDeer_return_rate() + (agent.getDeer_hunted() * Village.PROTEIN_PER_DEER) / (deer_time*Village.WORK_CAL_MAN));
		...
		if (hare_time > 0) {
			agent.setHare_return_rate(0);
			agent.setHare_return_rate(agent.getHare_return_rate() + (agent.getHare_hunted() * Village.PROTEIN_PER_HARE) / (hare_time*Village.WORK_CAL_MAN));
		...
		if (rabbit_time > 0) {
			agent.setRabbit_return_rate(0);
			agent.setRabbit_return_rate(agent.getRabbit_return_rate() + (agent.getRabbit_hunted() * Village.PROTEIN_PER_RABBIT) / (rabbit_time*Village.WORK_CAL_MAN));
	...
	public int searchHuntatX(int dx, int dy) {
		...
		long hh_protein_need = 0;		// grams needed for hh to survive (roughly 20 g/indiv/day)
		int hunt_kcal = 0;       //Numbers of Calories used for hunting
		int i_protein_storage;

		int Protein_Need = agent.getMySwarm().getProteinNeed();
		int hunt_radius = agent.getMySwarm().getHuntingRadius();
		...
		double trips;
		double tpercent_found;
		double tpercent_kill;
		double tpercent_chance;
		...
		// calculate protein need for hh per year to survive (roughly 20g per indiv per day)
		hh_protein_need = Protein_Need * agent.getFamilySize() * 365;  // 2 adults 12g + 4 children avg 6g / 6 = 8g
		i_protein_storage = 0;
		...
		do {
			...
			if (agent.getHsearchradius() < 51) {
				...
				if (dw == 0) {
					...
					hunt_kcal += agent.calcTravelCal(agent.getHsearchradius(), 0, 1.5);
					...
					if (known_deer != 0) {
						do {
							...
							if (known_deer == 0) {
								//accrue costs of hunting
								hunt_kcal += 4 * numhunts * Village.WORK_CAL_MAN;

								//add costs for retrieval
								trips = (killed_deer * 36) / Village.CARRY_CAPACITY;
								hunt_kcal += agent.calcTravelCal(agent.getHsearchradius(), 0, trips);
							}
						} while (known_deer != 0);
					}
				} else {
						...
						if (known_hares > Village.HLAG_HUNT) {
							do {
								...
								if (hpercent_chance > hpercent_kill) {
									...
									killed_hares += Village.HLAG_HUNT;
									known_hares -= Village.HLAG_HUNT;
									i_protein_storage += (455.4 * Village.HLAG_HUNT);
									if (i_protein_storage > hh_protein_need) {
										known_hares = 0;
									}
								}
								...
								if (known_hares < Village.HLAG_HUNT) {
									...
									hunt_kcal += 0.5 * numhunts * Village.WORK_CAL_MAN;
									if (Village.HUNTERS > 1) {
										hunt_kcal += 0.5 * numhunts * Village.WORK_CAL_WOM;
										for (i = 0; i < (Village.HUNTERS - 2); i++) {
											hunt_kcal += 0.5 * numhunts * Village.WORK_CAL_KID;
										}
									}
						...
						if (known_rabbits > Village.RLAG_HUNT) {
							do {
								...
								if (rpercent_chance > rpercent_kill) {
									...
									killed_rabbits += Village.RLAG_HUNT;
									known_rabbits -= Village.RLAG_HUNT;
									i_protein_storage += (177.2 * Village.RLAG_HUNT);
									if (i_protein_storage > hh_protein_need) {
										known_rabbits = 0;
									}
								}
			...
			if (agent.getHsearchradius() == hunt_radius) {
				//penalty for moving to a location where hunting is more than 5 km distant

				hunt_kcal += 250000;//was 1000000
				cont_hunting = 0;
				agent.setHunt_test_penalty(1);
			}
